...
- [ ] **WS2 (gsql/) IN PROGRESS (branch `ws2-gsql`, worktree `/home/teak/code/wt-gsql`)** — all 19 `.gsql` files written and were installing/running cleanly against the live full-dataset graph (10 contract queries, 5 detectors, 4 algorithms, discovery report), including the card-id-mismatch cases (`find_prior_cases` on real card `C15620-K1` correctly surfaces stub-referenced case `CC-0001`) and a fixed WCC hub-collapse issue (see `docs/decisions.md`). **Blocked mid-verification**: the TigerGraph Docker container/image disappeared from the environment (not caused by WS2) right as `discovery_report.gsql`'s last fix was being re-tested — needs the container/image restored, then a full `make verify-gsql` (not yet wired into the Makefile) and `tests/ws2/*.test.ts` (not yet written) before this can be called done. `docs/IDENTITY_VALIDATION.md` also not yet written. See `docs/logs.md` for the full blow-by-blow and `docs/decisions.md` for the design calls made along the way.
