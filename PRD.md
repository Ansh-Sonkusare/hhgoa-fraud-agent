...
| OQ7 | Agent framework and LLM | **Brief: framework is participant's choice (LangChain/LangGraph/OpenAI Agents SDK/CrewAI/custom) — we choose custom (justified in section 5). LLM is participant's choice, "for reasoning, tool selection, evidence synthesis, explanations — not replacing graph analysis" — we use Claude via `@anthropic-ai/sdk` (TS); local embeddings via `@xenova/transformers` running `bge-small-en-v1.5` in Node, no key needed** |
